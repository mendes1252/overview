...
function MiniProgressBar({ value, color = "#4A9FFF" }: { value: number; color?: string }) {
  return (
    <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
      <div
        className="h-full rounded-full animate-progress-fill"
        style={{ "--progress-width": `${value}%`, backgroundColor: color } as React.CSSProperties}
      />
    </div>
  );
}
    ...
    <div className="h-full bg-[#1A1A2E] phone-screen-content">
      ...
      <div className="px-3 pb-4 space-y-2">
        {tasks.map((t, i) => (
          <div
            key={i}
            className="flex items-start gap-2.5 p-2.5 rounded-xl bg-white/[0.05] border border-white/[0.06]"
          >
            <div
              className={`w-4 h-4 rounded-full border-2 mt-0.5 flex items-center justify-center shrink-0 ${
                t.done
                  ? "bg-[#4A9FFF] border-[#4A9FFF]"
                  : "border-white/20"
              }`}
            >
    ...
    <div className="h-full bg-[#1A1A2E] phone-screen-content">
      ...
      <div className="px-3 pb-4 space-y-2.5">
        {habits.map((h, i) => (
          <div
            key={i}
            className="p-3 rounded-xl bg-white/[0.05] border border-white/[0.06]"
          >
            <div className="flex items-center justify-between mb-2.5">
              ...
              <span className="text-[9px] bg-orange-500/15 text-orange-400 px-1.5 py-0.5 rounded-md flex items-center gap-0.5">
            ...
            <div className="grid grid-cols-7 gap-1">
              {h.days.map((done, d) => (
                <div key={d} className="text-center">
                  <span className="text-[8px] text-white/30 block mb-0.5">
                    {dayLabels[d]}
                  </span>
                  <div
        ...
        {goals.map((g, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-[10px] text-white/50 font-medium w-8 text-right">
                {g.progress}%
              </span>
            </div>
          </div>
...
export function CoachScreen() {
  const messages = [
    {
      from: "ai",
      text: "Bom dia! Vi que você completou 94% das tarefas essa semana. Excelente!",
    },
    {
      from: "user",
      text: "Obrigado! Alguma dica para manter o ritmo?",
    },
    {
      from: "ai",
      text: "Seu pico de foco é entre 9h-11h. Tente agendar tarefas difíceis nesse horário. E continue com a meditação matinal — faz diferença nos seus dados!",
    },
  ];
            ...
            >
              {m.from === "ai" && (
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-[8px] text-[#4A9FFF] font-medium">
                    Coach IA
                  </span>
                </div>
              )}
      ...
      <div className="px-3 pb-4 space-y-2.5">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <div
              key={i}
              className="bg-white/[0.05] rounded-xl p-2.5 text-center border border-white/[0.06]"
            >
              <s.icon className="w-3 h-3 text-[#4A9FFF] mx-auto mb-1" />
              <div className="text-sm font-medium text-[#4A9FFF]">
                {s.value}
              </div>
        ...
        <div className="p-3 rounded-xl bg-white/[0.05] border border-white/[0.06]">
          <div className="flex items-center gap-1 mb-1.5">
            <Sparkles className="w-2.5 h-2.5 text-[#4A9FFF]" />
            <span className="text-[9px] text-[#4A9FFF] font-medium">
              Insight da IA
            </span>
          </div>
          <p className="text-[10px] text-white/60 leading-relaxed">
            &ldquo;Você completou 15% mais tarefas de alta prioridade esta
            semana. Seu pico de foco é entre 9h-11h.&rdquo;
          </p>
        </div>
        {/* Recommendations */}
        <div className="p-3 rounded-xl bg-[#4A9FFF]/[0.08] border border-[#4A9FFF]/[0.12]">
          <span className="text-[9px] text-[#4A9FFF] font-medium block mb-1.5">
            Recomendações
          </span>
          <ul className="space-y-1">
            {[
              "Agendar tarefas difíceis entre 9h-11h",
              "Manter streak de meditação",
            ].map((r, i) => (
              <li
                key={i}
                className="text-[9px] text-white/50 flex items-center gap-1.5"
              >
    ...
    <div className="h-full bg-[#1A1A2E] phone-screen-content">
      ...
      <div className="px-3 pb-4 space-y-2">
        ...
        <div className="p-2.5 rounded-xl bg-white/[0.05] border border-white/[0.06]">
          ...
          <MiniProgressBar value={80} />
        </div>
        {/* Today tasks */}
        <div className="p-2.5 rounded-xl bg-white/[0.05] border border-white/[0.06]">
          <span className="text-[9px] text-white/40 mb-1.5 block">
            Tarefas de Hoje
          </span>
          {["Revisar proposta", "Reunião 14h", "Enviar relatório"].map(
            (t, i) => (
